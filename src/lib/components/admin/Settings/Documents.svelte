<script lang="ts">
	...
	const toggleContextlessContinuation = async () => {
		querySettings.contextlessContinuation = !querySettings.contextlessContinuation;
		querySettings = await updateQuerySettings(localStorage.token, querySettings);
	};
...
>
	<div class=" space-y-2.5 overflow-y-scroll scrollbar-hidden h-full pr-1.5">
		<div class="flex flex-col gap-0.5">
			...
			<div class=" flex w-full justify-between">
				<div class=" self-center text-xs font-medium">{$i18n.t('Contextless continuation')}</div>

				<button
					class="p-1 px-3 text-xs flex rounded transition"
					on:click={() => {
						toggleContextlessContinuation();
					}}
					type="button"
				>
					{#if querySettings.contextlessContinuation === true}
						<span class="ml-2 self-center">{$i18n.t('On')}</span>
					{:else}
						<span class="ml-2 self-center">{$i18n.t('Off')}</span>
					{/if}
